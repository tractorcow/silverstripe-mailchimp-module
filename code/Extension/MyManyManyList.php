...
class MyManyManyList extends ManyManyList{
	...
	*/
	...
	public function onLink($item) {
	    ...
	    // Send E-mail Confirmation of Event Attendance Update (Blind Copy in JBBA Admin)
        $email = new Email();
		$email->setFrom('noreply@basketball.je');
        $email->setBcc('admin@basketball.je');
        $email->setTo($member->Email);
        $email->setSubject('JBBA - Event Attendance Updated');
